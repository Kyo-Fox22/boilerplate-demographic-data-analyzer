...
def calculate_demographic_data(print_data=True):
    ...
    higher_education = df['education'].str.contains(r'Bachelors|Masters|Doctorate')
    ...
    highest_earning_country = df.groupby('native-country').agg({'salary':lambda x: (x == '>50K').sum()/len(x) * 100}).sort_values(by='salary', ascending=False).index[0]
    highest_earning_country_percentage = round(df.groupby('native-country').agg({'salary':lambda x: (x == '>50K').sum()/len(x) * 100}).sort_values(by='salary', ascending=False).loc['Iran','salary'],1)
